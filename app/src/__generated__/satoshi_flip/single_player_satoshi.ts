...
/**
 * The core logic of the Satoshi Flip game. Facilitates the creation of new games,
 * the distribution of funds, and the cancellation of games.
 */
...
const $moduleName = '@local-pkg/satoshi-flip::single_player_satoshi';
...
/**
 * Function used to create a new game. The player must provide a guess. Stake is
 * taken from the player's coin and added to the game's stake. The house's stake is
 * also added to the game's stake.
 */
export function startGame(options: StartGameOptions) {
    ...
    const parameterNames = ["guess", "coin", "houseData"];
...
export function guessPlacedEpoch(options: GuessPlacedEpochOptions) {
    const packageAddress = options.package ?? '@local-pkg/satoshi-flip';
    const argumentsTypes = [
        `${packageAddress}::single_player_satoshi::Game`
    ] satisfies string[];
